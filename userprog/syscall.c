...
bool
buffer_safe(const void* buff, unsigned size){

  if(!pointer_safe(buff)){
    return false;
  }

  unsigned buffer = (unsigned) buff;
  unsigned i;

  for(i=buffer; i < buffer + size; i++) {

    if(!pointer_safe((void*) i)){
      return false;
    }
  }
  return true;
